import Buttons from '../components/Buttons'

const Subcribe = () => {
  return (
    <section className="max-container flex 
    justify-between items-center max-lg:flex-col gap-10"
    id="contact-us">
      <h3 className="text-4xl leading-[68px] lg:max-w-md font-bold text-center">
        Sign up from <span className="text-coral-red">Updates</span> & Newsletter</h3>
      <div className='lg-max-w-[40%] w-full flex items-center max-sm:flex-c gap-5 p-2.5
      sm:border sm:border-slate-gray rounded-full'>
        <input type="text" placeholder="Subscribe to lemuel" className="input"/>
        <div className='flex max-sm:justify-end items-center max-sm:w-full'>
          <Buttons label="Sign Up"
          fullWidth/>
        </div>
      </div>
    </section>
  )
}

export default Subcribe